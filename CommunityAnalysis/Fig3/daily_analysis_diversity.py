...
def simper(fn, tax, rng, axis, x, xlim, ylim, colors, xtxt, ytxt, ylab):
    ...
    for a in range(len(axis)):
        ...
        ax.set_xlim(xlim)
        ax.set_ylim(ylim)
        ax.bar(x, treat_mean[a], yerr=treat_sd[a], color=colors, error_kw=dict(ecolor='gray', lw=1, capsize=3, capthick=1, alpha=0.5))
        title = old[a]+'\n'
        if otus[a] == 'Bacteria':
...
def barplot(fn, tax, lim, ax, alpha):
    ...
    for j in range(1, data.shape[0]):
        ax.bar(x, data[j], color=colors[j], bottom=bottom[j-1], label=taxa[j], alpha=alpha)
    ax.set_ylim([0, 100])
    if ax == l16S:
...
def get_diversity_plot(fn, ax,fn2, ax2, diversity='Simpsons'):
    ...
    x = [1, 2, 3, 4]
    y = [1, 1, 1, 1]
    ax.bar(x, y, color=colors1[0], width=1.0)
    #plt.setp(ax, xticks=[1.4, 2.4, 3.4, 4.4], xticklabels=[1, 2, 3, 4])
    plt.setp(ax.get_yticklabels(), visible=False)
    plt.setp(ax.get_xticklabels(), visible=False)
    ax.set_xlim([1, 5])
    ax2.tick_params(axis='x',which='both',top='off', bottom='off')
    ax2.bar(x, y, color=colors1[1], width=1.0)
...
simper(sim_16S, tax_16S, 5, s16S, [1, 2, 3, 4], [0.75, 5], [0, 40], cols_16S, 1, 35, '16S')
simper(sim_18S, tax_18S, 5, s18S, [1, 2, 3, 4], [0.75, 5], [0, 100], cols_18S, 1, 86, '18S')
...
lAb = lA.get_bbox_patch()
lAb.set_boxstyle("circle")
lB = l16S.text(-0.98, 99, 'B', fontsize=16, fontweight='bold', color='k', bbox=dict(facecolor='white', edgecolor='white'))
lBb = lB.get_bbox_patch()
lBb.set_boxstyle("circle")
lC = l16S.text(-0.98, -18, 'C', fontsize=16, fontweight='bold', color='k', bbox=dict(facecolor='white', edgecolor='white'))
lCb = lC.get_bbox_patch()
lCb.set_boxstyle("circle")
#l16S.text(7.5, 102.5, 'Simpsons Index of\nDiversity', size=8, ha='center')

l16S.set_ylabel('Relative abundance (%)')
l16S.set_xlabel('Days')
l18S.set_xlabel('Days')
s3_18S.set_xlabel('Days')
s1_16S.text(-1.2, 20, '16S rRNA gene', fontsize=10, ha='center', va='center', rotation=90)
s1_18S.text(-1.4, 50, '18S rRNA gene', fontsize=10, ha='center', va='center', rotation=90)
