...
def barplot(fn, tax, lim, ax, alpha, colors):
    ...
    if colors == colors_16S:
        ax.bar(x, data[0], color=colors[0], label=taxa[0], alpha=0.2, edgecolor='k', linewidth=0.7)
        for j in range(1, data.shape[0]):
            if j == 2 or j == 3:
                ax.bar(x[:2], data[j][:2], color=colors[j], bottom=bottom[j-1][:2], label=taxa[j], alpha=1, edgecolor='k', linewidth=2)
                ax.bar(x[2:], data[j][2:], color=colors[j], bottom=bottom[j-1][2:], label=taxa[j], alpha=0.2, edgecolor='k', linewidth=0.7)
            else:
                ax.bar(x, data[j], color=colors[j], bottom=bottom[j-1], label=taxa[j], alpha=0.2, edgecolor='k', linewidth=0.7)
    elif colors == colors_18S:
        ax.bar(x, data[0], color=colors[0], label=taxa[0], alpha=0.2, edgecolor='k', linewidth=0.7)
        for j in range(1, data.shape[0]):
            if j == 0 or j == 1:
                ax.bar(x[:2], data[j][:2], color=colors[j], bottom=bottom[j-1][:2], label=taxa[j], alpha=0.2, edgecolor='k', linewidth=0.7)
...
plt.savefig('Pres1 16S and 18S daily '+diversity+'p3.png', bbox_inches='tight', dpi=600)
