...
def transform_for_NMDS(df):
    ...
    print(nmds.stress_) 
    """Note that unless an update has been made to sklearn, this is a raw
    stress value and not a normalized stress value (i.e. outside of the range of 0 to 1)
    See: https://github.com/Borchmann/scikit-learn/blob/stress1/sklearn/manifold/mds.py"""
    # Rescale the data
...
def plot_nmds(fn, fnc, ax):
    ...
    for b in range(len(controls[0])):
        b += count
        ax.scatter(npos[b,0], npos[b,1], marker='o', color='k')
    """
    ...
    if ax == ax2:
        ax.legend(bbox_to_anchor=(1.5, 1.03), fontsize=6, scatterpoints=1)
...
ax1.text(-90, 80, 'A', fontsize=18, weight='bold')
ax1.text(-90, -130, 'B', fontsize=18, weight='bold')
...
ax2.set_ylabel('')
ax1.set_title('16S rDNA', fontsize=16)
ax2.set_title('18S rDNA', fontsize=16)
