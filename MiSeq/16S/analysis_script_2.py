...
def get_means(percent):
    ...
    for g in range(len(mean_values)):
        ...
        kstats = stats.kruskal(numpy.array(mean_values[g][0]), numpy.array(mean_values[g][1]), numpy.array(mean_values[g][2]), numpy.array(mean_values[g][3]))
        krusk_stats.append(kstats[0])
        krusk_p.append(kstats[1])
    nfn = percent[:-4]+'_mean.csv'
...
names, indices, all_samples, all_treatments = analysis_script_1.get_meta_treatments('Plastics_meta.csv') 

simper = ['Plastic_simper.csv']
percent = ['Plastic_percent.csv']
all_files = ['Plastic_percent.csv']
...
name = get_values_shared(all_OTUs, percent[0], all_treatments[0], all_samples[0])
get_means(name)
